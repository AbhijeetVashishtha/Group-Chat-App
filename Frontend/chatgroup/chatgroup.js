...
async function creategroup(event){
   try{
    ...
    const response = await axios.post('http://13.233.91.195:3000/group/creategroup', groupDetails, {headers: {'Authorization': token}});
...
window.addEventListener('DOMContentLoaded', async () => {
    try{
        const response = await axios.get('http://13.233.91.195:3000/group/getallgroups', {headers: {'Authorization': token}});
